...
def enhance_medical_text(text, doc_type):
    ...
    response = openai.ChatCompletion.create(
        ...
        messages=[
            ...
            {"role": "user", "content": f"Correct this medical document OCR output:\n{text}"}
        ],
        temperature=0.1,
        max_tokens=1024
    )
...
# Process a file
def process_file(file_path, model, processor, output_dir):
    ...
    if file_type and file_type.startswith('image/'):
        ...
        ocr_text = docx_to_text(file_path)
        doc_type = detect_document_type(ocr_text)
        enhanced_text = enhance_medical_text(ocr_text, doc_type)
    else:
    ...
    output_file = os.path.join(output_dir, f"{os.path.basename(file_path)}_{doc_type}.txt")
...
def process_directory(input_dir, output_dir):
    ...
    supported_extensions = ['.jpg', '.jpeg', '.png', '.bmp', '.pdf', '.docx']
    ...
    for file_path in all_files:
        try:
            process_file(file_path, model, processor, output_dir)
        except Exception as e:
            print(f"Error processing {file_path}: {e}")

# Main function
def main():
    input_dir = r"C:\Users\<your_username>\Desktop\input"
    output_dir = r"C:\Users\<your_username>\Desktop\output"
...
# from transformers import AutoProcessor, AutoModelForCausalLM
# from PIL import Image
# import openai  # Import the entire module instead of specific class
# import mimetypes
# import numpy as np
# import glob
# from IPython.display import display, Markdown

# # Set the token directly in the code instead of loading from env
# HF_TOKEN = "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"
...
#         openai_api_key = "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"
